...
flowchart TD
    ...
    subgraph rust ["Phase 2: Rust (runs on every TAB)"]
        direction TB
        G["_cc_completer"] --> H["Read completion.toml"]
...
`_cc_completer`, a statically linked Rust binary. It reads the
...
completion system to `_cc_completer`. The scripts are generated by
...
2. The function calls `_cc_completer` with the current command line
