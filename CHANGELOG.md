...
- **Plugin-aware completions**: automatically discovers and completes all
  conda plugin subcommands (workspace, global, self, spawn, task, etc.)
  without manual configuration.
- **Contextual completions**: reads project files in the working directory
  to complete environment names, task names, feature names, and channels.
- **Broad file format support**: conda.toml, pixi.toml, pyproject.toml,
  environment.yml, anaconda-project.yml, conda-project.yml, conda.lock,
  pixi.lock, conda-lock.yml, .condarc, environments.txt, global.toml.
- **Stat-based file cache**: tracks (mtime, size) tuples for every source
  ...
  5ms.
- **Cross-shell support**: bash, zsh, PowerShell (Tier 1, fully tested),
  fish (Tier 2, community-tested). Descriptions shown alongside candidates
  in zsh, fish, and PowerShell.
- **Automatic manifest regeneration**: a `conda_post_commands` hook detects
  when plugins are installed or removed and silently regenerates the
  completion manifest.
- **Security hardened**: output sanitization strips control characters from
  candidates, file reads are size-limited (10MB), symlinks are rejected
  throughout the file walker and cache.
